...
class FrontToken {
  ...
  static final Lock _tokenInfoMutex = Lock();
  static Lock _frontTokenMutex = Lock();
    ...
    while (true) {
      ...
      if (frontToken == null) {
        ...
        if (localSessionState.status == LocalSessionStateStatus.EXISTS) {
          ...
          await _frontTokenMutex.synchronized(() async {
            // Wait for the lock and then retry
          });
        } else {
          logDebugMessage(
  ...
  static Future<bool> doesTokenExist() async {
    ...
    return await _frontTokenMutex.synchronized(() async {
  ...
  static Future<void> removeToken() async {
    ...
    await _tokenInfoMutex.synchronized(() async {
